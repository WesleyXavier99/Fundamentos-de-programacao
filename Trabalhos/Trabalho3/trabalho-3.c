...
int inserir(TLista *L,TLista *topo,int valor){
	...
	if(verificar(*L,valor)==1||valor==0){
		return FALSE;
	}else{
		//aloca memoria
			...
			*L = aux1;
			...
			while(aux2){
				...
				if(aux2->valor==valor&&aux2->prox==NULL){
					printf("\nremoveu da ultima posicao\n");
					if((*topo)->valor==aux2->valor&&aux2->prox==NULL){//se o topo for o ultimo elemento
						aux3->prox = aux2->prox;
						free(aux2);
						*topo=aux3;
						printf("\nremoveu no topo\n");
					...
					}else{/*eu to achando esse if e else desnecessario, era pro caso de o topo ser o meio e os positivos estarem na frente dele e não atras */
						printf("\nremoveu fora do topo\n");
						aux3 = aux2->prox;
						free(aux2);
						return TRUE;
					}
